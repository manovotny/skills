...
- **Also catch revised bot comments.** A bot that rewrites its summary in place (CodeRabbit does this) keeps its `id` and `created_at`, so the cursor above never sees it. For each bot-authored comment (`user.type == "Bot"`), compare its `updated_at` against `botRevisions["{stream}:{id}"]`: an entry that is missing or stale means new bot feedback. Treat those as activity alongside the cursor-selected comments, labeled as a revision rather than a new comment, then write the fresh `updated_at` back.
