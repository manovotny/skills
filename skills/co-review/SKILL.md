...
2. **Make the fixes locally.** Do NOT commit or push yet. Let the user review the changes first.
   - **Prose fixes carry the voice.** When a fix lands in prose that lives in the repo — docs, READMEs, blog posts, changelogs, release notes — write it per `~/.claude/skills/co-write/voice.md` if it exists, using the medium overlay that matches the content (Docs, Blog & long-form, Release notes & changelogs, ...; core alone when none fits). Apply the guide directly; don't invoke `/co-write`. A local styleguide or repo convention outranks the voice, and voice never widens the fix: revoice only the prose the fix already touches.
3. **Verify the fixes before showing them.** These are changes you authored and will push, so confirm they hold up — don't claim merge-ready on faith. Run the repo's fast checks on the change — **formatter, linter, type check, and tests** — when they're detectable and runnable. Detect the commands; never hardcode them. Prefer what CI runs (read CI config), then `package.json` scripts, `Makefile`/`justfile`, then the ecosystem's standard (`pyproject.toml`/`tox`/`ruff`, `cargo`, `go`, etc.).
   ...
   - **Self-check touched prose.** Formatters and linters don't read wording. Re-read every prose line the fix touched against `voice.md`, read fresh rather than recalled, and fix what fails. Never show the user prose that would flunk `/co-write check`.
