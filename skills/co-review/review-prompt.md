...
- A finding that confirms a bot reviewer's comment adds one field to the standard format so the reply lands in-thread:

  Bot thread: {bot login}, root comment id {id}

- Close with a **Skipped bot comments** list — `{bot login}, root comment id {id}: {one-line skip reason}` — so every triaged bot thread carries a verdict. Omit the list when there are no bot threads.
