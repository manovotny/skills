...
**Gather context with the tools you already have — never escalate to reach it.** The toolkit is `gh`, `git`, reachable MCP servers, already-installed CLIs, and local files. Many links in a PR description (Linear, Notion, dashboards, private or unrelated repos) are auth-gated and won't resolve; when a reference doesn't open with those tools, record it as unavailable context and move on. Do **not** launch browser automation, install packages, or attempt to bypass a sandbox or filesystem restriction to reach a source — the PR diff plus reachable repo sources are enough to review, and missing external context is a note, not a blocker. A source that would settle an objective claim but isn't reachable is reported as unverifiable (Claude asks the user to provide it; Codex, which runs non-interactively, flags it in the findings) — never grounds for building a workaround.
