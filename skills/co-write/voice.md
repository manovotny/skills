...
- Emojis are a language on Slack, and the one place they belong: message templates (reviews, team updates, team metrics), emoji reactions, emoji-only replies, and workflow markers (`:ready-for-review:` + linked PR title + "→ one-line context"). Inside a template they carry meaning — status markers like `:linear-todo:` / `:linear-completed:`, section anchors — never decoration. Skip ornamental `:wave:`-style emoji, especially in greetings; the decorative ones dilute the meaningful ones.
