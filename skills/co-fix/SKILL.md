...
Then pull both PR comment streams (`gh api --paginate repos/{owner}/{repo}/pulls/{pr}/comments` and `gh api --paginate repos/{owner}/{repo}/issues/{pr}/comments`) and collect **bot reviewer threads** — authors with `user.type == "Bot"` raising code concerns (CodeRabbit, Cursor's Bugbot, the ChatGPT/Codex connector, Macroscope, …); skip non-review bot noise (deploy previews, coverage summaries, changelog bots) and any thread that already has a human reply. Re-fetch every round — bots often comment while the loop runs. Bot findings are candidates for Step 6's filter, not obligations. Record each thread's root comment id for the Step 8b reply.
...
**Step 8b — Reply to bot reviewer threads.** After the push, close the loop on every bot thread triaged this round — one reply per thread, posted autonomously (co-fix already commits and pushes unattended):

- **Fixed** → "Fixed in abc1234." — plain-text hash so GitHub renders the commit link.
- **Dismissed** → the skip reason, brief and direct — not apologetic.
...
Reply to the thread's **root comment id** (from Step 4). Actionable findings from a top-level bot comment have no thread — answer them in one combined `gh pr comment` instead. Rounds 2+ only answer bot comments that arrived since the last round; never re-reply to a thread already answered. Write replies first person as the user; if `~/.claude/skills/co-write/voice.md` exists, apply it (medium: PR & review comments).
