...
- **Reviewer proposals are not requirements.** A guarantee Codex proposes — or a fix from an earlier round happens to introduce — is not an acceptance criterion unless the user or an existing repo contract requires it. Judge each round's findings against the PR's original scope, not against proposals from previous rounds; more rounds never promote a proposal into a requirement. This is what keeps the loop from ratcheting: round 2 suggests a guarantee, round 3 reviews against it as if it were required, and by the cap the PR has grown machinery nobody asked for.
